...
class FastEdit(nn.Module):
    def __init__(self, handler):
        super(FastEdit, self).__init__()
        self.topo_encoder = None
        self.set_topo_encoder(handler)
        self.gt_layers = nn.Sequential(*[TransfLayer() for i in range(args.gt_layer)])
...
class TransfLayer(nn.Module):
    def __init__(self):
        ...
        self.Q = nn.Parameter(init(t.empty(args.latdim, args.head, args.latdim//args.head)))
        self.K = nn.Parameter(init(t.empty(args.latdim, args.head, args.latdim//args.head)))
        self.V = nn.Parameter(init(t.empty(args.latdim, args.head, args.latdim//args.head)))
        self.att_linear = FeedForwardLayer(args.latdim, args.latdim, bias=False)
        self.dense_layers = nn.Sequential(*[FeedForwardLayer(args.latdim, args.latdim, bias=True, act=args.act) for i in range(2)])
        self.layer_norm1 = nn.LayerNorm(args.latdim)
        self.layer_norm2 = nn.LayerNorm(args.latdim)
    ...
    def forward(self, embeds):
        ...
        anchor_embeds = self._attention(anchor_embeds, embeds) + anchor_embeds
        embeds = self._attention(embeds, anchor_embeds) + embeds
        embeds = self.layer_norm1(embeds)
        embeds = self.dense_layers(embeds) + embeds
        embeds = self.layer_norm2(embeds)
        return embeds
