...
class Coach:
    ...
    def run(self):
        ...
        self.handler.trn_loader.dataset.neg_sampling()
        start_time = time()
        self.model.unlearn((self.handler.torch_uni_adj, self.handler.torch_adj))
        if args.finetune_steps > 0:
            reses = self.trn_epoch(args.finetune_steps)
            log(self.make_print('Fine-tune', 0, reses, True))
        log('Unlearning time %.3f' % (time() - start_time))
    ...
    def prepare_model(self):
        self.model = FastEdit(self.handler).cuda()
    ...
    def trn_epoch(self, finetune_steps):
        ...
        for i, tem in enumerate(trn_loader):
            ...
            loss.backward()
            self.opt.step()
            log('Step %d/%d: loss = %.6f, regLoss = %.6f         ' % (i, finetune_steps, loss, reg_loss), save=False, oneline=True)
        ret = dict()
...
if __name__ == '__main__':
    ...
    log('Start')
    handler = DataHandler()
    handler.load_data(args.unlearn_rate)
    log('Load Data')
